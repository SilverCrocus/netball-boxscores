...
const GENERIC_SIGN_IN_ERROR = 'Sign in could not be completed. Please try again or use your email and password.';
...
function signInErrorMessage(code: string | null): string {
  ...
  switch (code.toLowerCase()) {
    case 'oauthaccountnotlinked':
      return 'An account already exists with this email. Sign in with the same method you used when creating it.';
    case 'accessdenied':
      return 'Google sign-in was cancelled or access was denied. Please try again or use your email and password.';
    case 'configuration':
      return 'Google sign-in is temporarily unavailable. Please use your email and password or try again later.';
...
function SignInForm() {
  ...
  const queryError = signInErrorMessage(searchParams.get('error'));
  ...
  useEffect(() => {
    ...
    void getProviders()
      .then((providers) => {
        if (!cancelled) setHasGoogleProvider(Boolean(providers?.google));
      })
      .catch(() => {
