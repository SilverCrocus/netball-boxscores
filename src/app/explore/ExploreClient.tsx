...
function EvidencePanel({ model, response }: { model: ExploreResultModel; response: StatQueryResponse }) {
  return (
    <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <article className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5 sm:p-6"><p className="font-label text-[11px] font-black uppercase tracking-[0.16em] text-secondary">Included matches</p><h3 className="mt-2 font-headline text-xl font-black text-primary">Audit the sample</h3>{model.includedMatchIds.length > 0 ? <div className="mt-4 flex flex-wrap gap-2">{model.includedMatchIds.map((matchId, index) => <Link key={matchId} href={`/match/${matchId}`} className="rounded-lg border border-outline-variant bg-surface-container-low px-3 py-2 font-label text-xs font-bold text-primary hover:border-secondary">Match {index + 1}</Link>)}</div> : <p className="mt-3 text-sm text-on-surface-variant">No match-level identifiers were returned for this aggregate.</p>}<details className="mt-5 border-t border-outline-variant pt-4 text-xs text-on-surface-variant"><summary className="cursor-pointer font-bold text-primary">Normalized query</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap break-words rounded-lg bg-primary-container p-3 text-[10px] text-slate-200">{JSON.stringify(response.spec, null, 2)}</pre></details></article>
    </section>
  );
}
